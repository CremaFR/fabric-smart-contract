...
describe('Account-account@1.0.0' , () => {
    ...
    const fabricWallet: fabricNetwork.FileSystemWallet = new fabricNetwork.FileSystemWallet(
        '/Users/thomasbrillard/Documents/Project/fabric/fabcar-blockchain-sample/web-app/server/wallet');
    let identityName: string;
    let connectionProfile: any;

    before(async () => {
        const connectionProfilePath: string = '/Users/thomasbrillard/Documents/Project/fabric/fabcar-blockchain-sample/web-app/server/mychannel_fabcar_profile.json';
    ...
    it('getAccount', async () => {
        ...
        const response: Buffer = await submitTransaction('getAccount', args);
    ...
    it('addAccount', async () => {
        ...
        const response: Buffer = await submitTransaction('addAccount', args);
        // submitTransaction returns buffer of transcation return value
        // TODO: Update with return value of transaction
        assert.equal(true, true);
        // assert.equal(JSON.parse(response.toString()), undefined);
    ...
    it('revoke', async () => {
        ...
        const response: Buffer = await submitTransaction('revoke', args);
        // submitTransaction returns buffer of transcation return value
        // TODO: Update with return value of transaction
        assert.equal(true, true);
        // assert.equal(JSON.parse(response.toString()), undefined);
    ...
    async function submitTransaction(functionName: string, args: string[]): Promise<Buffer> {
        ...
        const contract: fabricNetwork.Contract = await network.getContract('account');
